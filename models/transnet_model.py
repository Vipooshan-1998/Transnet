...
## Without Spatial Trans for MLWA Ablation
import torch
import torch.nn as nn
from torch_geometric.nn import (
    TransformerConv,
...
class Trans_LSTM_Sans_Spatial_Trans(nn.Module):
    def __init__(self, input_dim=2048, embedding_dim=128, img_feat_dim=2048, num_classes=2):
        super(Trans_LSTM_Sans_Spatial_Trans, self).__init__()
        ...
        # -----------------------
        # Temporal graph transformer only
        # -----------------------
        self.gc1_temporal = TransformerConv(
            in_channels=embedding_dim * 2 + embedding_dim // 2,
            out_channels=embedding_dim // 2,
        ...
        self.gc1_norm2 = InstanceNorm(
            (embedding_dim // 2) * self.graph_heads
        )
        ...
        # -----------------------
        self.temporal_lstm_graph = nn.LSTM(
            input_size=(embedding_dim // 2) * self.graph_heads,
            hidden_size=(embedding_dim // 2) * self.graph_heads,
            num_layers=1,
            batch_first=True
        )
    ...
    def forward(self, x, edge_index, img_feat, video_adj_list,
        ...
        x_feat = self.relu(
            self.x_bn1(
                self.x_fc(x[:, :self.input_dim])
            )
        )

        x_label = self.relu(
            self.obj_l_bn1(
                self.obj_l_fc(x[:, self.input_dim:])
            )
        )
        ...
        g_embed_seq, _ = self.temporal_lstm_graph(
            g_embed_seq
        )
        ...
        img_feat_seq, _ = self.temporal_lstm_img(
            img_feat_seq
        )

        lstm_out_img = img_feat_seq.squeeze(0)

        lstm_out_img = sanitize(
            lstm_out_img,
            "lstm_out_img"
        )
        ...
        # Classification
        # -----------------------
        fused_feat = self.relu(
            self.classify_fc1(fused_feat)
        )

        logits_mc = self.classify_fc2(
            fused_feat
        )

        probs_mc = self.softmax(
            logits_mc
        )
...
## Without Temporal Trans for MLWA Ablation
import torch
import torch.nn as nn
from torch_geometric.nn import (
    TransformerConv,
...
class Trans_LSTM_Sans_Temporal_Trans(nn.Module):
    def __init__(self, input_dim=2048, embedding_dim=128, img_feat_dim=2048, num_classes=2):
        super(Trans_LSTM_Sans_Temporal_Trans, self).__init__()
        ...
        # -----------------------
        # Spatial graph transformer only
        # -----------------------
        self.gc1_spatial = TransformerConv(
            in_channels=embedding_dim * 2 + embedding_dim // 2,
            out_channels=embedding_dim // 2,
        ...
        self.classify_fc1 = nn.Linear(
            concat_dim,
            embedding_dim
        )

        self.classify_fc2 = nn.Linear(
            embedding_dim,
            num_classes
        )
...
## Without Object LSTM for MLWA Ablation
import torch
import torch.nn as nn
from torch_geometric.nn import (
    TransformerConv,
...
class Trans_LSTM_Sans_Object_LSTM(nn.Module):
    def __init__(self, input_dim=2048, embedding_dim=128, img_feat_dim=2048, num_classes=2):
        super(Trans_LSTM_Sans_Object_LSTM, self).__init__()
        ...
        # Spatial and temporal graph transformers
        # -----------------------
        self.gc1_spatial = TransformerConv(in_channels=embedding_dim * 2 + embedding_dim // 2,
                                           out_channels=embedding_dim // 2,
                                           heads=self.graph_heads,
                                           edge_dim=1,
                                           beta=True)
        ...
        self.gc1_temporal = TransformerConv(in_channels=embedding_dim * 2 + embedding_dim // 2,
                                            out_channels=embedding_dim // 2,
                                            heads=self.graph_heads,
                                            edge_dim=1,
                                            beta=True)
    ...
    def forward(self, x, edge_index, img_feat, video_adj_list,
        ...
        # -----------------------
        # Fusion (Graph LSTM removed)
        # -----------------------
        fused_feat = torch.cat((g_embed, lstm_out_img), dim=1)
        fused_feat = sanitize(fused_feat, "fused_feat before classification")
