...
class SpaceTempGoG_detr_dad(nn.Module):
    ...
    def forward(self, x, edge_index, img_feat, atten_feat, video_adj_list, edge_embeddings,
        ...
        frame_embed_img = self.relu(self.gc2_norm_i3d(self.gc2_i3d(
            self.temporal_transformer_img(self.img_fc(img_feat).unsqueeze(0)).squeeze(0), video_adj_list.clone().long())))

        # -----------------------
        # Attention feature processing
        # -----------------------
        frame_embed_atten = self.relu(self.gc2_norm_atten(self.gc2_atten(
            self.temporal_transformer_atten(self.atten_fc(atten_feat).unsqueeze(0)).squeeze(0), video_adj_list.clone().long())))
